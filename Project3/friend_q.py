...
for i in range(iterations):
        ...
        if epsilon > np.random.random():
            ...
            player_A_action = np.argmax(sum(Q[current_playerA, current_playerB, ball]))

            player_B_action = np.argmin(sum(Q[current_playerA, current_playerB, ball]))
        ...
        Q[current_playerA, current_playerB, ball, player_A_action, player_B_action] = (1 - alpha) * Q[current_playerA, current_playerB, ball, player_A_action, player_B_action] + alpha * ((1 - gamma) * Rew_B + gamma * np.max(Q[player_A_pos, player_B_pos, game_ball]))
        epsilon *=decay
        alpha *= decay
